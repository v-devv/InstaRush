...
const Cart = () => {
    const {
        ...
        axios, getCartAmount, setCartItems
    } = useAppContext();
    ...
    const placeOrder = async () => {
        try {
            ...
            if (paymentOption === "COD") {
                ...
                toast("Online payment not implemented yet", { icon: "💳" });
            }
        } catch (error) {
            toast.error(error.message);
        }
            ...
            <div className="max-w-[360px] w-full bg-gray-100/40 p-5 max-md:mt-16 border border-gray-300/70">
                ...
                <div className="mb-6">
                    ...
                    <div className="relative flex justify-between items-start mt-2">
                        ...
                        <button onClick={() => setShowAddress(!showAddress)} className="text-purple-500 hover:underline cursor-pointer">Change</button>
                        {showAddress && (
                            <div className="absolute top-12 py-1 bg-white border border-gray-300 text-sm w-full z-10">
                                {addresses.map((address, i) => (
                                    <div key={i} className="flex justify-between items-center">
                                        <p onClick={() => { setSelectedAddress(address); setShowAddress(false); }} className="cursor-pointer text-gray-500 p-2 hover:bg-gray-100">
                                            {address.street}, {address.city}, {address.state}, {address.country}
                                        </p>
                                        <button onClick={() => confirm("Are you sure you want to delete this address?") && deleteAddress(address._id)} className="flex items-center text-red-500 pr-2">
